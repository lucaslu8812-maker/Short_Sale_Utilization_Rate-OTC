...
def get_valid_date(offset_start=1):
    ...
    for i in range(offset_start, offset_start + 30):
        ...
        try:
            url = f"https://www.tpex.org.tw/web/stock/margin_trading/loan_sbl/margin_sbl_result.php?l=zh-tw&d={int(d[:4])-1911}/{d[4:6]}/{d[6:]}&o=json"
...
# ===== 借券資料（含 retry + 正確欄位）=====
def get_borrow(date):
    roc_date = f"{int(date[:4])-1911}/{date[4:6]}/{date[6:]}"
    url = f"https://www.tpex.org.tw/web/stock/margin_trading/loan_sbl/margin_sbl_result.php?l=zh-tw&d={roc_date}&o=json"
    ...
    for _ in range(3):
        try:
            ...
            df.columns = [
                "證券代號","證券名稱",
                "融券賣出","融券買進","融券現券","融券今日餘額","融券限額",
                "前日餘額","當日賣出","當日還券","當日調整數額","當日餘額","借券賣出限額","備註"
            ]
